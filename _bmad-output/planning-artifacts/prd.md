...
**Resolution:** Minh sửa thao tác hoặc click vào link từ điển ngoại (Google/Cambridge/Oxford) được gợi ý để tra cứu thành công, giảm bực bội so với lỗi im lặng.
...
- FR21: Hệ thống có thể hiển thị trạng thái `not found` kèm hướng dẫn thao tác lại và các link tìm kiếm từ điển ngoại (Google, Cambridge, Oxford).
...
- FR31: Hệ thống có thể tạo link tìm kiếm ngoại cho Google, Cambridge, Oxford dựa trên token đã chuẩn hóa.
- FR32: Các link tìm kiếm ngoại phải được mở trong tab mới với thuộc tính bảo mật (`target="_blank"`, `rel="noopener noreferrer"`).
