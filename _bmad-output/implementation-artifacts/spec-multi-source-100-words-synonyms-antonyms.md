...
status: 'done'
baseline_commit: 'ca4841575d3734c8ef4bb63c9d70a54676206b45'
context: []
---
...
**Execution:**
- [x] `src/infrastructure/adapters/freeDictionaryApiAdapter.js` -- Format synonyms & antonyms đi kèm từng sense/definition và trích xuất danh sách tổng hợp.
- [x] `src/infrastructure/adapters/vocabularyHtmlParserAdapter.js` -- Format synonyms & antonyms đi kèm từng definition trong cụm "Definitions of..." và trích xuất danh sách tổng hợp.
- [x] `src/application/popupViewModelMapper.js` -- Đưa `synonyms` và `antonyms` vào view model.
- [x] `src/content/popupRenderer.js` -- Bổ sung render item cho tab `synonyms-antonyms`.
- [x] `src/content/popupManager.js` -- Render tab `Synonyms & Antonyms` (dạng chips tương tác) và hỗ trợ style inline synonyms/antonyms trong tab Definitions.
- [x] `tests/fixtures/commonWordsDataset.js` -- Cập nhật dataset 100 từ cho cả 2 nguồn.
- [x] `tests/e2e/multiSource100WordsDisplay.test.js` -- Chạy kiểm thử tự động 100 từ trên cả 2 nguồn.
...
- Trích xuất Synonyms & Antonyms từ cấu trúc `meanings` / `definitions` và gắn trực tiếp vào từng thẻ `<li>`.
  [`freeDictionaryApiAdapter.js:120`](../../src/infrastructure/adapters/freeDictionaryApiAdapter.js#L120)

- Trích xuất Synonyms & Antonyms từ cụm "Definitions of..." của Vocabulary.com và gắn vào `<li>`.
  [`vocabularyHtmlParserAdapter.js:150`](../../src/infrastructure/adapters/vocabularyHtmlParserAdapter.js#L150)

**View Model Mapping & Popup Rendering**

- Chuyển tiếp các mảng `synonyms` và `antonyms` vào ViewModel của Popup.
  [`popupViewModelMapper.js:57`](../../src/application/popupViewModelMapper.js#L57)

- Khai báo item type `synonyms-antonyms` cho Popup Renderer.
  [`popupRenderer.js:58`](../../src/content/popupRenderer.js#L58)

- Render Tab "Synonyms & Antonyms" với giao diện chip trực quan có thể click để tra từ.
  [`popupManager.js:2330`](../../src/content/popupManager.js#L2330)

**Dataset Fixture & Automated E2E Verification**

- Generator `generateFreeDictionaryJson` và cập nhật `generateVocabularyHtml` cho 100 từ phổ biến.
  [`commonWordsDataset.js:115`](../../tests/fixtures/commonWordsDataset.js#L115)

- Test suite chạy kiểm tra toàn diện 100 từ cho cả Vocabulary.com và FreeDictionary.
  [`multiSource100WordsDisplay.test.js:15`](../../tests/e2e/multiSource100WordsDisplay.test.js#L15)
