...
- **Quy tắc an toàn & Git Policy**:
  ...
  - **Quy trình Nâng Version & Release**: Khi người dùng yêu cầu "nâng version" hoặc tương tự, thực hiện: (1) Đọc toàn bộ lịch sử git kể từ tag gần nhất bằng `git describe --tags --abbrev=0` và `git log <latest_tag>..HEAD`, (2) In kế hoạch ra màn hình và chờ phê duyệt, (3) Cập nhật version trong `package.json` và `manifest.json`, (4) Ghi changelog đầy đủ vào `CHANGELOG.md`, (5) Kiểm tra `npm test` & build `npm run build`, (6) Tạo commit, đóng tag `vX.Y.Z` và push nếu được yêu cầu.
